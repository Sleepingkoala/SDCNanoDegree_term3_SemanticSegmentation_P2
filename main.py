...
# Core of this project - encoder and decoder of FCN
def layers(vgg_layer3_out, vgg_layer4_out, vgg_layer7_out, num_classes):
    ...
    # TODO: Implement function    
    '''
    According to the section 4.3 of original paper, there are three types of FCN nets: single-stream, two-stream and three-stream nets. Single stream net upsamples layer7 predictions at stride 32 back to input pixels in a single step. Similarly, two-stream net combines predictions from both last layer7 and layer4, at stride16. And three-stream net adds layer3 output, at stride 8. Here we apply the three-stream net with the highest accuracy, following step 1-8.
    '''
    ...
    layer7_upsamplex2 = tf.layers.conv2d_transpose(layer7_conv_1x1, num_classes, 4, strides = (2,2), 
                                                      padding = 'same',
                                                      kernel_initializer = tf.random_normal_initializer(stddev=0.01),                                                  
                                                      kernel_regularizer = tf.contrib.layers.l2_regularizer(1e-3)) 
    ...
    layer4_7_sum= tf.add(layer7_upsamplex2, layer4_conv_1x1)
    # step 5. upsampling by transposed convolution at stride (2,2)
    layer4_7_upsamplex2 = tf.layers.conv2d_transpose(layer4_7_sum, num_classes, 4, strides = (2,2),
                                                     padding = 'same',
                                                     kernel_initializer = tf.random_normal_initializer(stddev=0.01),
                                                     kernel_regularizer = tf.contrib.layers.l2_regularizer(1e-3))
    ...
    layer3_4_7_sum = tf.add(layer4_7_upsamplex2, layer3_conv_1x1)
    # step 8. upsampling by transposed convolution at stride (8,8), then comes the final layer output.
    three_stream_output = tf.layers.conv2d_transpose(layer3_4_7_sum, num_classes, 16, strides = (8,8),
                                                     padding = 'same',
                                                     kernel_initializer = tf.random_normal_initializer(stddev = 0.01),
                                                     kernel_regularizer = tf.contrib.layers.l2_regularizer(1e-3))    
    
    return three_stream_output

tests.test_layers(layers)

# Adam optimizer - less hyperparameters
def optimize(nn_last_layer, correct_label, learning_rate, num_classes):
    ...
    """
    # TODO: Implement function 
    # reshape 4D output tensor into 2D logits, where each row represents a pixel and each column a class
    logits = tf.reshape(nn_last_layer, (-1, num_classes))
    correct_label = tf.reshape(correct_label, (-1, num_classes))
    # pixelwise cross entropy loss function
    cross_entropy_loss = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(logits = logits,
                                                                                labels = correct_label))
    # Adam optimizer
    optimizer = tf.train.AdamOptimizer(learning_rate = learning_rate)
    # use optimizer to loss function, to get tf operations for this FCN training
...
def run():
    ...
    with tf.Session() as sess:
        ...
        correct_label = tf.placeholder(tf.int32,[None, None, None, num_classes], name = 'correct_label')  # tf placeholders
